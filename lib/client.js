	...
	factory: (require) => {
		...
		*/
		function SettingsPage(props) {
			if (props.view === "summary") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: props.t("rowSummary") });
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ProviderEditor, { ...props });
		}
		/**
		...
		*/
		function ProviderEditor({ form, t, send, useAigcSettings, saveProviders }) {
			const snapshot = useAigcSettings((s) => s);
			const providers = snapshot.status === "ready" && snapshot.value !== void 0 ? snapshot.value.providers : [];
			...
			(0, react.useEffect)(() => {
				if (status !== "ready" || value === void 0) return;
				setDrafts(toDrafts(value.providers));
			}, [status, revision]);
			const commit = (0, react.useCallback)(async (next) => {
				setError(void 0);
				try {
			...
			const add = (0, react.useCallback)(async () => {
				...
				if (await commit([...providers, stored])) {
			...
			const remove = (0, react.useCallback)(async (id) => {
				if (await commit(providers.filter((p) => p.id !== id))) {
			...
			if (form === void 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("section", {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				...
				children: [
					...
					status === "ready" && !snapshot.writable && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						className: SettingsPage_module_css_default.intro,
					...
					}) : status === "unavailable" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						className: SettingsPage_module_css_default.empty,
						children: t("settingsUnavailable")
					...
					status === "ready" && !addingNew && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						type: "button",
		...
		*/
		...
		const dicts = {
			ja: {
				...
				emptyHint: "プラグインページ(Plugins)のこのプラグインの行でプロバイダーを設定してから、エージェントに生成を依頼してください。",
				prompt: "プロンプト",
				image: "画像",
				video: "動画",
				audio: "音声",
				...
				uploading: "アップロード中…",
				rowSummary: "AIGC 生成プロバイダーを設定します(API アドレス、キー、認証方式、呼び出し手順)。",
				...
				settingsError: "エラー",
				settingsUnavailable: "設定サービスを利用できません（ホストがこのプラグインの設定を提供していません）。",
				settingsReadOnly: "この接続では設定を保存しないため、変更は保存されません。",
				settingsSaveFailed: "保存できませんでした。設定が他の場所で変更されました。再試行してください。",
				"row.id": "ID",
				"row.name": "名前",
			...
			de: {
				...
				emptyHint: "Konfiguriere Anbieter in der Zeile dieses Plugins auf der Plugins-Seite und bitte den Agenten dann, etwas zu generieren.",
				prompt: "Prompt",
				image: "Bild",
				video: "Video",
				audio: "Audio",
				...
				uploading: "Hochladen…",
				rowSummary: "AIGC-Generierungsanbieter konfigurieren (API-Adresse, Schlüssel, Authentifizierung, Aufrufanweisungen).",
				...
				settingsUnavailable: "Einstellungen sind nicht verfügbar (der Host stellt die Konfiguration dieses Plugins nicht bereit).",
				settingsReadOnly: "Diese Verbindung speichert die Konfiguration nicht; Änderungen werden nicht gespeichert.",
				settingsSaveFailed: "Speichern abgelehnt — die Konfiguration wurde anderweitig geändert. Bitte erneut versuchen.",
				"row.id": "ID",
				"row.name": "Name",
			...
			fr: {
				...
				emptyHint: "Configurez les fournisseurs dans la ligne de ce plugin sur la page Plugins, puis demandez à l’agent de générer quelque chose.",
				prompt: "Invite",
				image: "Image",
				video: "Vidéo",
				audio: "Audio",
				...
				rowSummary: "Configurer les fournisseurs de génération AIGC (endpoint API, clé, schéma d’authentification, instructions d’appel).",
				...
				settingsUnavailable: "Les paramètres sont indisponibles (l’hôte ne fournit pas la configuration de ce plugin).",
				settingsReadOnly: "Cette connexion ne persiste pas la configuration ; les modifications ne seront pas enregistrées.",
				settingsSaveFailed: "Enregistrement refusé — la configuration a été modifiée ailleurs. Veuillez réessayer.",
				"row.id": "ID",
				"row.name": "Nom",
				"row.endpoint": "Endpoint",
				"row.apiKey": "Clé API",
			...
			pt: {
				...
				emptyHint: "Configure provedores na linha deste plugin na página Plugins e peça ao agente para gerar algo.",
				prompt: "Prompt",
				image: "Imagem",
				video: "Vídeo",
				audio: "Áudio",
				...
				uploading: "Enviando…",
				rowSummary: "Configurar provedores de geração AIGC (endereço da API, chave, autenticação, instruções de chamada).",
				...
				settingsError: "Erro",
				settingsUnavailable: "As configurações estão indisponíveis (o host não serve a configuração deste plugin).",
				settingsReadOnly: "Esta conexão não persiste a configuração; as alterações não serão salvas.",
				settingsSaveFailed: "Salvamento recusado — a configuração foi alterada em outro lugar. Tente novamente.",
				"row.id": "ID",
				"row.name": "Nome",
			...
			ko: {
				...
				emptyHint: "플러그인(Plugins) 페이지의 이 플러그인 행에서 공급자를 구성한 다음 에이전트에게 생성을 요청하세요.",
				prompt: "프롬프트",
				image: "이미지",
				video: "동영상",
				audio: "오디오",
				...
				uploading: "업로드 중…",
				rowSummary: "AIGC 생성 공급자를 구성합니다(API 주소, 키, 인증 방식, 호출 지침).",
				...
				settingsError: "오류",
				settingsUnavailable: "설정 서비스를 사용할 수 없습니다(호스트가 이 플러그인의 구성을 제공하지 않음).",
				settingsReadOnly: "이 연결은 구성을 저장하지 않으므로 변경 사항이 저장되지 않습니다.",
				settingsSaveFailed: "저장이 거부되었습니다. 구성이 다른 곳에서 변경되었습니다. 다시 시도하세요.",
				"row.id": "ID",
				"row.name": "이름",
				"row.endpoint": "API 주소",
				"row.apiKey": "API 키",
			...
			ar: {
				...
				emptyHint: "قم بتكوين المزوّدين في صف هذا البرنامج المساعد على صفحة الإضافات (Plugins)، ثم اطلب من الوكيل توليد شيء.",
				prompt: "الموجّه",
				image: "صورة",
				video: "فيديو",
				audio: "صوت",
				...
				rowSummary: "تكوين مزوّدي توليد AIGC (عنوان API، المفتاح، المصادقة، تعليمات الاستدعاء).",
				...
				settingsError: "خطأ",
				settingsUnavailable: "الإعدادات غير متاحة (لا يوفر المضيف تكوين هذا البرنامج المساعد).",
				settingsReadOnly: "هذا الاتصال لا يحفظ التكوين؛ لن يتم حفظ التغييرات.",
				settingsSaveFailed: "تم رفض الحفظ — تم تغيير التكوين في مكان آخر. يرجى المحاولة مرة أخرى.",
				"row.id": "المعرّف",
				"row.name": "الاسم",
			...
			hi: {
				...
				emptyHint: "प्लगइन्स (Plugins) पेज पर इस प्लगइन की पंक्ति में प्रदाता कॉन्फ़िगर करें, फिर एजेंट से कुछ जनरेट करने के लिए कहें।",
				prompt: "प्रॉम्प्ट",
				image: "छवि",
				video: "वीडियो",
				audio: "ऑडियो",
				...
				rowSummary: "AIGC जनरेशन प्रदाता कॉन्फ़िगर करें (API पता, कुंजी, प्रमाणीकरण, कॉल निर्देश)।",
				...
				settingsUnavailable: "सेटिंग्स अनुपलब्ध हैं (होस्ट इस प्लगइन का कॉन्फ़िगरेशन प्रदान नहीं करता है)।",
				settingsReadOnly: "यह कनेक्शन कॉन्फ़िगरेशन सहेजता नहीं है; परिवर्तन सहेजे नहीं जाएंगे।",
				settingsSaveFailed: "सहेजना अस्वीकार — कॉन्फ़िगरेशन कहीं और बदल गया। कृपया पुनः प्रयास करें।",
				"row.id": "आईडी",
				"row.name": "नाम",
			...
			id: {
				...
				emptyHint: "Konfigurasikan penyedia di baris plugin ini pada halaman Plugins, lalu minta agen menghasilkan sesuatu.",
				prompt: "Prompt",
				image: "Gambar",
				video: "Video",
				audio: "Audio",
				...
				rowSummary: "Konfigurasikan penyedia pembuatan AIGC (alamat API, kunci, autentikasi, instruksi pemanggilan).",
				...
				settingsUnavailable: "Pengaturan tidak tersedia (host tidak menyajikan konfigurasi plugin ini).",
				settingsReadOnly: "Koneksi ini tidak menyimpan konfigurasi; perubahan tidak akan disimpan.",
				settingsSaveFailed: "Penyimpanan ditolak — konfigurasi berubah di tempat lain. Coba lagi.",
				"row.id": "ID",
				"row.name": "Nama",
			...
			tr: {
				...
				emptyHint: "Eklentiler (Plugins) sayfasında bu eklentinin satırında sağlayıcıları yapılandırın, sonra aracıdan bir şey üretmesini isteyin.",
				prompt: "İstek",
				image: "Görsel",
				video: "Video",
				audio: "Ses",
				...
				rowSummary: "AIGC üretim sağlayıcılarını yapılandırın (API adresi, anahtar, kimlik doğrulama, çağrı talimatları).",
				...
				settingsError: "Hata",
				settingsUnavailable: "Ayarlar kullanılamıyor (ana makine bu eklentinin yapılandırmasını sunmuyor).",
				settingsReadOnly: "Bu bağlantı yapılandırmayı kalıcı kılmaz; değişiklikler kaydedilmez.",
				settingsSaveFailed: "Kaydetme reddedildi — yapılandırma başka yerde değişti. Lütfen tekrar deneyin.",
				"row.id": "Kimlik",
				"row.name": "Ad",
			...
			vi: {
				...
				emptyHint: "Cấu hình nhà cung cấp trong hàng plugin này trên trang Plugins, rồi yêu cầu agent tạo nội dung.",
				prompt: "Lời nhắc",
				image: "Hình ảnh",
				video: "Video",
				audio: "Âm thanh",
				...
				rowSummary: "Cấu hình nhà cung cấp tạo AIGC (địa chỉ API, khóa, xác thực, hướng dẫn gọi).",
				...
				settingsError: "Lỗi",
				settingsUnavailable: "Cài đặt không khả dụng (máy chủ không cung cấp cấu hình của plugin này).",
				settingsReadOnly: "Kết nối này không lưu cấu hình; thay đổi sẽ không được lưu.",
				settingsSaveFailed: "Đã từ chối lưu — cấu hình đã thay đổi ở nơi khác. Vui lòng thử lại.",
				"row.id": "ID",
				"row.name": "Tên",
			...
			th: {
				...
				emptyHint: "กำหนดค่าผู้ให้บริการในแถวของปลั๊กอินนี้บนหน้าปลั๊กอิน (Plugins) แล้วให้เอเจนต์สร้างสิ่งที่ต้องการ",
				prompt: "พรอมต์",
				image: "รูปภาพ",
				video: "วิดีโอ",
				audio: "เสียง",
				...
				rowSummary: "กำหนดค่าผู้ให้บริการสร้าง AIGC (ที่อยู่ API, คีย์, การยืนยันตัวตน, คำแนะนำการเรียก)",
				...
				settingsUnavailable: "การตั้งค่าใช้งานไม่ได้ (โฮสต์ไม่ได้ให้ค่าตั้งค่าของปลั๊กอินนี้)",
				settingsReadOnly: "การเชื่อมต่อนี้ไม่บันทึกค่าตั้งค่า การเปลี่ยนแปลงจะไม่ถูกบันทึก",
				settingsSaveFailed: "บันทึกไม่สำเร็จ — ค่าตั้งค่าถูกเปลี่ยนที่อื่น โปรดลองอีกครั้ง",
				"row.id": "ID",
				"row.name": "ชื่อ",
			...
			ru: {
				...
				emptyHint: "Настройте провайдеров в строке этого плагина на странице Plugins, затем попросите агента что-нибудь сгенерировать.",
				prompt: "Промпт",
				image: "Изображение",
				video: "Видео",
				audio: "Аудио",
				...
				uploading: "Загрузка…",
				rowSummary: "Настройка провайдеров генерации AIGC (адрес API, ключ, аутентификация, инструкции вызова).",
				...
				settingsUnavailable: "Настройки недоступны (хост не предоставляет конфигурацию этого плагина).",
				settingsReadOnly: "Это соединение не сохраняет конфигурацию; изменения не будут сохранены.",
				settingsSaveFailed: "Сохранение отклонено — конфигурация изменилась в другом месте. Повторите попытку.",
				"row.id": "ID",
				"row.name": "Название",
			...
			it: {
				...
				emptyHint: "Configura i provider nella riga di questo plugin nella pagina Plugin, poi chiedi all’agente di generare qualcosa.",
				prompt: "Prompt",
				image: "Immagine",
				video: "Video",
				audio: "Audio",
				...
				rowSummary: "Configura i provider di generazione AIGC (indirizzo API, chiave, autenticazione, istruzioni di chiamata).",
				...
				settingsUnavailable: "Le impostazioni non sono disponibili (l’host non serve la configurazione di questo plugin).",
				settingsReadOnly: "Questa connessione non salva la configurazione; le modifiche non verranno salvate.",
				settingsSaveFailed: "Salvataggio rifiutato — la configurazione è cambiata altrove. Riprova.",
				"row.id": "ID",
				"row.name": "Nome",
				"row.endpoint": "Endpoint",
			...
			nl: {
				...
				emptyHint: "Configureer providers in de rij van deze plugin op de Plugins-pagina en vraag de agent dan iets te genereren.",
				prompt: "Prompt",
				image: "Afbeelding",
				video: "Video",
				audio: "Audio",
				...
				rowSummary: "AIGC-generatieproviders configureren (API-adres, sleutel, authenticatie, aanroepinstructies).",
				...
				settingsError: "Fout",
				settingsUnavailable: "Instellingen zijn niet beschikbaar (de host levert de configuratie van deze plugin niet).",
				settingsReadOnly: "Deze verbinding slaat de configuratie niet op; wijzigingen worden niet bewaard.",
				settingsSaveFailed: "Opslaan geweigerd — de configuratie is elders gewijzigd. Probeer het opnieuw.",
				"row.id": "ID",
				"row.name": "Naam",
			...
			sv: {
				...
				emptyHint: "Konfigurera leverantörer i denna plugins rad på Plugins-sidan och be sedan agenten generera något.",
				prompt: "Prompt",
				image: "Bild",
				video: "Video",
				audio: "Ljud",
				...
				rowSummary: "Konfigurera AIGC-genereringsleverantörer (API-adress, nyckel, autentisering, anropsinstruktioner).",
				...
				settingsError: "Fel",
				settingsUnavailable: "Inställningarna är inte tillgängliga (värden tillhandahåller inte pluginens konfiguration).",
				settingsReadOnly: "Denna anslutning sparar inte konfigurationen; ändringar sparas inte.",
				settingsSaveFailed: "Spara nekades — konfigurationen ändrades någon annanstans. Försök igen.",
				"row.id": "ID",
				"row.name": "Namn",
			...
			pl: {
				...
				emptyHint: "Skonfiguruj dostawców w wierszu tej wtyczki na stronie Wtyczki (Plugins), a następnie poproś agenta o wygenerowanie czegoś.",
				prompt: "Komunikat",
				image: "Obraz",
				video: "Wideo",
				audio: "Audio",
				...
				rowSummary: "Skonfiguruj dostawców generowania AIGC (adres API, klucz, uwierzytelnianie, instrukcje wywołania).",
				...
				settingsError: "Błąd",
				settingsUnavailable: "Ustawienia są niedostępne (host nie udostępnia konfiguracji tej wtyczki).",
				settingsReadOnly: "To połączenie nie zapisuje konfiguracji; zmiany nie zostaną zapisane.",
				settingsSaveFailed: "Odrzucono zapis — konfiguracja zmieniła się gdzie indziej. Spróbuj ponownie.",
				"row.id": "ID",
				"row.name": "Nazwa",
			...
			"zh-HK": {
				...
				emptyHint: "可在外掛頁(Plugins)本外掛的行內設定供應商,然後讓模型開始生成。",
				prompt: "提示詞",
				image: "圖片",
				video: "影片",
				audio: "音頻",
				...
				dropHint: "拖放檔案到畫布",
				uploading: "上載中…",
				rowSummary: "設定 AIGC 生成供應商(API 位址、金鑰、鑑權方式、呼叫說明)。",
				...
				settingsLoading: "載入中…",
				settingsError: "錯誤",
				settingsUnavailable: "設定服務無法使用（主機未提供此外掛程式的設定）。",
				settingsReadOnly: "此連線不會保存設定，變更將不會儲存。",
				settingsSaveFailed: "儲存失敗：設定已在其他地方修改，請稍後重試。",
				"row.id": "ID",
				"row.name": "名稱",
				"row.endpoint": "API 位址",
				"row.apiKey": "API 金鑰",
		...
		*/
		...
		const ENTRY_ID = "dsh-aigc-canvas";
		/** `plugins.row.config` key: the bundle's package name `#` the patch row id. */
		const ROW_CONFIG_KEY = `@huanlin/dsh-plugin-aigc-canvas#${ENTRY_ID}`;
		//#endregion
		...
		*  `configForms` is provided by the ui-settings base plugin. */
		...
		function apply(ctx) {
			...
			const settingsInjected = () => ({
				...
				saveProviders: (providers) => form.set("providers", providers.map((p) => ({
					...p,
					auth: { ...p.auth }
				})))
			});
